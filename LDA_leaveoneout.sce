...
Base = backup
Teste_amostras = Base(nn,1:34);
Teste_rotulos = Base(nn,35);
Base(nn,:) = []
Treino_amostras = Base(:,1:34);
Treino_rotulos = Base(:,35);
...
Aux = zeros(6);
 ...
 for k=1:6
     Aux(k) = abs(abs(tm)-abs(decisao(k)));        
 end
 [A,B] = min(Aux);
 Rotulos_finais = B;
...
//Classificação
 ...
 if Rotulos_finais == Teste_rotulos;
     count = count +1;
 end
end

Resultado = count/amostras
